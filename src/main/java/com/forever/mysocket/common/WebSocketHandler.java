package com.forever.mysocket.common;
...
public class WebSocketHandler extends TextWebSocketHandler implements InitializingBean {
    ...
    public void handleMessage(WebSocketSession session, WebSocketMessage<?> message) throws Exception {
		super.handleMessage(session, message);
  		...
  		for (WebSocketSession sess : sessionSet) {
  			sess.sendMessage(new TextMessage("Client "+session.getId() + " : " + message.getPayload()));
  		}
    }
    ...
    public void afterPropertiesSet() throws Exception {
        Thread thread = new Thread(){
            int i=0;
            String A = "[응급상황발생] ";
            String B = "GPS : 127.0.0.1:8080 ";
            String C = "ID : 123456(컨트롤러) "; 
            
            SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss ");
            String format_date = date.format(System.currentTimeMillis());
            @Override
            public void run() {
               while (true){
				  try {
					  	broadcast(A + format_date + C + B );
